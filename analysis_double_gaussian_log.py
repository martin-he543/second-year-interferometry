...
fig, ax = plt.subplots()
ax.set_title('Fully-Corrected Spectrum FFT: Zero-Crossing Analysis\n%s'%file, **titleFont)
ax.plot(new_x_data,new_y_data, label='After Shifting and Cubic-Spline\nN=%i, Full Hg'%(N))
ax.set_xlabel("Position ($\mu$steps)",**axesFont)
ax.set_ylabel('Intensity (a.u.)', **axesFont)
#ax.set_yticklabels(**ticksFont)
fig.legend(prop=font, loc="lower right")
ax.ticklabel_format(useMathText=True)
ax.tick_params(axis='both', labelsize=7)
ax.set_xlim(400,1000)
ax.set_ylim(3e5,1e10)
ax.set_yscale("log")
plt.show()
