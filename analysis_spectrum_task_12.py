...
file="data/Task_12_yellow_doublet_1.txt" #this is the data
...
values = [(True, True, True), (True, False, False), (False, True, False), (False, False, True)]

#new_y_data = new_y_data[np.argmax(new_y_data)]
...
plt.xlabel("Position / nm", **axesFont) #labelpad=30
plt.ylabel('Intensity / a.u.', **axesFont)
plt.xticks(**ticksFont)
plt.yticks(**ticksFont)
plt.plot(new_x_data,new_y_data, label='After Shifting and Cubic-Spline\nN=%i, Entire Hg Spectrum'%(N))
# State which section to annotate with values.
sub = bool(values[0]); mid = bool(values[1]); sup = bool(values[2])
...
i = 0
print(i)
if i == 0:      plt.xlim(0,2400)
elif i == 1 :   plt.xlim(0,400)
elif i == 2 :   plt.xlim(400,1000)
else:           plt.xlim(0,2400)
plt.ylim(0,1e10)
#plt.savefig("FFT_spectrum_" + file + "["+ str(xlim_values[0]) + "," + str(xlim_values[1]) + "].png", dpi=1000)
plt.show()
